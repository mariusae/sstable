...
-- support *strict* bytestrings only, write directly.  *then*, we can
-- build a data.binary interface on top of this.

-- we always write bytestring -> bytestring (strict)
...
-- *TODO* Writing couldd be done by continuation or iteratees. Let's
-- take this approach first & improve upon it later.  (Same with the
-- scan interface.)

-- The on-disk format here is very simple:
-- 
--   the first 8 bytes stores the OFFSET to the index.
--   the next OFFSET - 8 bytes stores the values. these are 
--     length:u64,bytes
--   at byte OFFSET 
  ...
  withFile path WriteMode $ \fh -> do
    ...
    (_, index) <- 
      foldM' (0, []) records $ \(n, index) (k, v) -> do
        let !encoded = {-# SCC "encode-v" #-} encode v
            l        = L.length encoded
            !this    = (k, n, fromIntegral l)
        {-# SCC "hPut-v" #-} L.hPut fh encoded
    ...
    {-# SCC "put-i" #-} put fh (listArray (0, (length index) - 1) (reverse index) :: Index)
    {-# SCC "put-io" #-} put fh (fromIntegral indexOffset :: Int64)
