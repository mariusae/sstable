...
makeSearchList l = nubBy (\(a, _) (b, _) -> a == b) (sort l)
...
prop_binary_search_works (IA l) = go ((snd . bounds) index)
  where
    ...
    go (-1) = True
    go n = leastUpperBound index (fst $ index!n) == Just n && (go $ n - 1)
